...
1. **จำแนกและเปรียบเทียบ (Classify & Compare)** ความแตกต่างเชิงคุณสมบัติระหว่าง List, Tuple, Set และ Dictionary (Mutability, Ordering, Uniqueness)
2. **ประยุกต์ใช้ (Apply)** Tuple สำหรับการจัดเก็บข้อมูลพิกัดที่ไม่ต้องการให้เปลี่ยนแปลง (Immutable Data) และเทคนิค Packing / Unpacking
3. **ดำเนินการทางคณิตศาสตร์ (Perform Set Operations)** กับ Set เช่น ยูเนียน อินเตอร์เซกชัน ผลต่าง และผลต่างสมมาตร
4. **ออกแบบฐานข้อมูลจำลอง (Design Hash Lookups)** ด้วย Dictionary เพื่อการค้นหาข้อมูลในเวลาคงที่ $O(1)$ ผ่านระบบ Hash Table

---

## 🌌 4.0 เรื่องเล่าเปิดบทเรียน: มนต์วิเศษของตารางแฮช (Hash Tables)
...
| **มีลำดับ (Ordered)** | ✅ ใช่ | ✅ ใช่ | ❌ ไม่การันตีลำดับ | ✅ ใช่ (ตามลำดับที่แทรก) |
| **แก้ไขค่าได้ (Mutable)** | ✅ แก้ไขได้ | ❌ แก้ไขไม่ได้ (Immutable) | ✅ แก้ไขได้ | ✅ แก้ไขได้ |
| **สมาชิกซ้ำได้ (Duplicates)**| ✅ ซ้ำได้ | ✅ ซ้ำได้ | ❌ สมาชิกต้องไม่ซ้ำกัน | ❌ คีย์ต้องไม่ซ้ำกัน |
| **การเข้าถึงข้อมูล** | ดัชนี `[0]` | ดัชนี `[0]` | ลูป หรือ `in` | คีย์ `["key"]` |
| **ค้นหาข้อมูล (`in`)** | **$O(n)$** | **$O(n)$** | **$O(1)$ ⚡** | **$O(1)$ ⚡** |

---

## 🔒 4.2 ทูเพิล (Tuple) และการกระจายตัวแปร (Unpacking)
...
## ⭕ 4.3 เซต (Set) และการดำเนินการทางทฤษฎีเซต
...
## 📖 4.4 พจนานุกรม (Dictionary) และระบบฐานข้อมูลตารางธาตุ
